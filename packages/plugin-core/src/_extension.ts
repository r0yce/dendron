...
import { setupCommands } from "./extension/setupCommands";
import {
  setupCriticalLanguageFeatures,
  setupDeferredLanguageFeatures,
} from "./extension/setupLanguageFeatures";
...
export async function _activate(
  ...
  try {
    ...
    // Setup the commands
    await setupCommands({ ext: ws, context, requireActiveWorkspace: false });
    ...
    await setupCommands({ ext: ws, context, requireActiveWorkspace: true });
    ...
    if (await DendronExtension.isDendronWorkspace()) {
      ...
      if (!opts?.skipLanguageFeatures) {
        setupCriticalLanguageFeatures(context);
        const handle = scheduleDeferred(
          () => {
            setupDeferredLanguageFeatures(context);
          },

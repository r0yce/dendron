import {
  ...
  getWebTreeViewEntry,
  ThemeType,
  WebViewThemeMap as CommonWebViewThemeMap,
} from "@dendronhq/common-all";
import { inject, injectable } from "../../../di/inject";
import * as vscode from "vscode";
import { URI, Utils } from "vscode-uri";

type WebViewThemeMap = CommonWebViewThemeMap;

/**
 * Web-extension WebViewUtils (browser/webworker host).
 * HTML shell: pure `genVSCodeHTMLIndex` from `@dendronhq/common-all` (shared with desktop).
 */
@injectable()
...
<body>
   ...
   */
  /**
   * Pure shell from common-all (shared with desktop WebViewCommonUtils).
   */
  private genVSCodeHTMLIndex({
    ...
    return genVSCodeHTMLIndexPure({
      name,
      jsSrc,
      cssSrc,
      url,
      wsRoot: wsRoot.fsPath,
      browser,
      acquireVsCodeApi,
      themeMap,
      initialTheme,

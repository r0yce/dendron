...
class AutoCompleter {
     ...
     * */
    static autoCompleteNoteLookup(currentValue, activeItemValue, fnames) {
        if (fnames.length === 0) {
            return currentValue;
        }
        let topPickIdx = 0;
        ...
        if (common_all_1.FuseEngine.doesContainSpecialQueryChars(currentValue)) {
    ...
    static matchPrefixTillNextDot(topPick, value, minIdx) {
    ...
    static matchNoteUpToCurrValue(topPick, currentValue) {
    ...
    static getAutoCompletedValue(_quickPick) {
        ...
        const fnames = _.uniq(_quickPick.items
            // Remove create new since it does not make sense for name auto completion.
            .filter((item) => !constants_1.CREATE_NEW_DETAIL_LIST.includes(item.detail || ""))
            .map((item) => item.fname));

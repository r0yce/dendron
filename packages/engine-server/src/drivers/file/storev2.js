...
class FileStorage {
    ...
    async findNotes(opts) {
        ...
        if (fname) {
            ...
            notes = lodash_1.default.values(this.notes).filter((note) => common_all_1.VaultUtils.isEqualV2(note.vault, vault));
        }
        else {
            notes = lodash_1.default.values(this.notes);
        }
        if (excludeStub) {
            notes = notes.filter((note) => note.stub !== true);
        }
        return lodash_1.default.cloneDeep(notes);
    }
    /**
     *
    ...
    async deleteNote(id, opts) {
        ...
        if (!lodash_1.default.isEmpty(noteToDelete.children)) {
            ...
            if (parentNote) {
                ...
                common_all_1.DNodeUtils.removeChild(parentNote, noteToDelete);
                common_all_1.DNodeUtils.addChild(parentNote, replacingStub);
                out.push({
                    note: parentNote,
                    status: "update",
    ...
    async _initSchema(vault) {
        ...
        const out = await (0, common_server_1.getAllFiles)({
            root: vscode_uri_1.URI.file(vpath),
            include: ["*.schema.yml"],
        });
    ...
    async initNotes() {
        ...
        if (this.config.workspace.metadataStore === "sqlite") {
            ...
            const store = new SQLiteMetadataStore_1.SQLiteMetadataStore({
                wsRoot: this.wsRoot,
                force: true,
            });
            // sleep until store is done
            const output = await common_all_1.TimeUtils.awaitWithLimit({ limitMs: 6e4 }, async () => {
                while (store.status === "loading") {
                    this.logger.info({ ctx, msg: "downloading sql dependencies..." });
                    // eslint-disable-next-line no-await-in-loop
                    await common_all_1.TimeUtils.sleep(1000);
                }
                return;
            });
            this.logger.info({
            ...
            if (!(await SQLiteMetadataStore_1.SQLiteMetadataStore.isDBInitialized())) {
                await SQLiteMetadataStore_1.SQLiteMetadataStore.createAllTables();
                await SQLiteMetadataStore_1.SQLiteMetadataStore.createWorkspace(this.wsRoot);
            }
        }
        const out = await Promise.all(this.vaults.map(async (vault) => {
    ...
    _addBacklinksImpl(allNotes, notesWithLinks) {
        ...
        notesWithLinks.forEach((noteFrom) => {
            ...
            try {
                ...
                this.logger.error({
                    error,
                    noteFrom: common_all_1.NoteUtils.toLogObj(noteFrom),
                    noteTo: _noteToForErrorLog
                        ? common_all_1.NoteUtils.toLogObj(_noteToForErrorLog)
                        : null,
    ...
    _addLinkCandidates() {
        return lodash_1.default.map(this.notes, (noteFrom) => {
            try {
    ...
    async processNoteChangedByRename({ note, oldLoc, newLoc, }) {
        ...
        if (oldLoc.fname.toLowerCase() === newLoc.fname.toLowerCase() &&
            ...
            allLinks = lodash_1.default.filter(allLinks, (link) => {
                ...
                if (link.to?.anchorHeader === oldLoc.anchorHeader)
                    return true;
                ...
                return (link.type === "ref" &&
                    ...
                    this.referenceRangeParts(link.to?.anchorHeader).includes(oldLoc.anchorHeader));
            });
        }
        ...
        allLinks = allLinks.filter((link) => {
            ...
            const implicitlySameVault = lodash_1.default.isUndefined(link.to?.vaultName) && lodash_1.default.isEqual(note.vault, oldLocVault);
        ...
        const noteMod = lodash_1.default.reduce(allLinks, (note, link) => {
            ...
            if (oldLink.from.alias && oldLink.from.alias !== oldLink.from.fname) {
                ...
                if (oldLoc.alias?.toLocaleLowerCase() ===
    ...
    async renameNote(opts) {
        ...
        if (!this.notes[noteRaw.id]) {
            throw new common_all_1.DendronError({
                ...
                message: `Unable to rename note "${noteRaw.fname}" in vault "${common_all_1.VaultUtils.getName(noteRaw.vault)}".` +
        ...
        const notesWithLinkTo = lodash_1.default.uniq(oldNote.links
            .filter((link) => link.type === "backlink")
            .map((link) => {
            if (link.from.id)
                return this.notes[link.from.id];
            else
                return undefined;
        })
            .filter(common_all_1.isNotUndefined));
        ...
        const notesToUpdate = (await Promise.all(notesWithLinkTo.map(async (note) => {
    ...
    async _writeNewNote({ note, existingNote, opts, }) {
        ...
        utils_1.EngineUtils.refreshNoteLinksAndAnchors({
            note,
            engine: this.engine,
    ...
    async writeNote(note, opts) {
        ...
        if (maybeNote) {
            ...
            const schemaMatch = await common_all_1.SchemaUtils.matchPath({
                notePath: note.fname,
                engine: this.engine,
            });
            if (schemaMatch) {
        ...
        if (maybeNote && maybeNote.id === note.id) {
             ...
             */
            const deletedLinks = maybeNote.links.filter((link) => link.to?.fname &&
                !note.links.some((linkToCompare) => common_all_1.DLinkUtils.isEquivalent(link, linkToCompare)));
            const addedLinks = note.links.filter((link) => link.to?.fname &&
                !maybeNote.links.some((linkToCompare) => common_all_1.DLinkUtils.isEquivalent(link, linkToCompare)));
    ...
    async writeSchema(schemaModule, opts) {
    ...
    async addBacklink(link) {
        ...
        if (maybeBacklink) {
            ...
            return Promise.all(notes.map(async (note) => {
                ...
                common_all_1.BacklinkUtils.addBacklinkInPlace({ note, backlink: maybeBacklink });
                return {
                    prevNote,
                    note,
                    status: "update",
    ...
    async removeBacklink(link) {
        ...
        if (maybeBacklink) {
            ...
            return Promise.all(notes.map(async (note) => {
                const prevNote = lodash_1.default.cloneDeep(note);
                common_all_1.BacklinkUtils.removeBacklinkInPlace({
                    note,
                    backlink: maybeBacklink,
                });
                return {

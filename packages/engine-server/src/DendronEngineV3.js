...
class DendronEngineV3 extends common_all_1.EngineV3Base {
    ...
    static create({ wsRoot, logger }) {
        ...
        const queryStore = new common_all_1.FuseQueryStore();
    ...
    async writeNote(note, opts) {
        ...
        if (existingNote && existingNote.id === note.id) {
             ...
             */
            const deletedLinks = existingNote.links.filter((link) => link.to?.fname &&
    ...
    async querySchema(queryString) {
        ...
        const responses = await this._schemaStore.bulkGetMetadata(schemaIds.value.map((ent) => ent.id));
    ...
    async getNoteBlocks(opts) {
        ...
        try {
            ...
            if (opts.filterByAnchorType) {
                lodash_1.default.remove(blocks, (block) => block.anchor?.type !== opts.filterByAnchorType);
            }
            return { data: blocks };
        }
        catch (err) {

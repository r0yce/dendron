/**
 * Theme CSS URLs injected into the webview HTML shell.
...
export type GenVSCodeHTMLIndexOpts = {
  name: string;
  jsSrc: string;
  cssSrc: string;
  url: string;
...
/**
 * Pure HTML shell for Dendron React webviews (desktop + web extension).
 * No Node/fs dependencies — safe for webworker bundles.
 *
 * Desktop: common-server WebViewCommonUtils delegates here.
 * Web extension: plugin-core web WebViewUtils delegates here.
 */
export function genVSCodeHTMLIndex({
  name,
  jsSrc,
  cssSrc,
  url,

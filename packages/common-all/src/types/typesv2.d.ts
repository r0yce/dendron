...
export type DNoteLinkRaw<TData extends DNoteLinkData = any> = Omit<DNoteLink<TData>, "from"> & {
    from?: DNoteLoc;
};
 ...
 */
export type DNodeOpts<T = any> = Partial<Omit<DNodeProps<T>, "fname|type|vault">> & {
    fname: string;
    type: DNodeType;
    vault: DVault;
};
export type SchemaRaw = Pick<SchemaProps, "id"> & Partial<SchemaData> & {
    title?: string;
    desc?: string;
...
export type StoreV2InitResp = RespWithOptError<DEngineInitPayload & {
...
export type DEngine = DCommonProps & DCommonMethods & {
    vaults: DVault[];
    hooks: DHookDict;
...
export type DStore = DCommonProps & DCommonMethods & {
    /**
     * @deprecated
...
export type OnDidChangeActiveTextEditorMsg = DMessage<"onDidChangeActiveTextEditor", OnDidChangeActiveTextEditorData>;
export type OnUpdatePreviewHTMLMsg = DMessage<DMessageEnum.ON_UPDATE_PREVIEW_HTML, OnUpdatePreviewHTMLData>;
export type GraphViewMessage = DMessage<GraphViewMessageType, {
    id: string;
    vault?: string;
...
export type ConfigureUIMessage = DMessage<ConfigureUIMessageType, {
    config: DendronConfig;
}>;
export type CalendarViewMessage = DMessage<CalendarViewMessageType, {
    id?: string;
    fname?: string;
}>;
export type NoteViewMessage = DMessage<NoteViewMessageType, {
    id?: string;
    href?: string;
}>;
export type SeedBrowserMessage = DMessage<SeedBrowserMessageType | DMessageEnum, {
    data: any;
}>;

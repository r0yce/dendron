...
class NoteLookupUtils {
    ...
    static fetchRootResults = (notes) => {
        const roots = __1.NoteUtils.getRoots(notes);
        const childrenOfRoot = roots.flatMap((ent) => ent.children);
        const childrenOfRootNotes = lodash_1.default.map(childrenOfRoot, (ent) => notes[ent]).filter(Boolean);
...
function regularTransform(trimmedQuery, onlyDirectChildren) {
    ...
    }; // TODO: proper type for strict optional properties after TS upgrade
}

...
class TemplateUtils {
    ...
    static async findAndApplyTemplate({ note, engine, pickNote, }) {
        ...
        if (maybeTemplate) {
            ...
            const { link: fname, vaultName } = (0, common_all_1.parseDendronURI)(maybeTemplate?.id);
    ...
    static genTrackPayload(templateNote) {
        const fnameToDate = templateNote.body.match(/\{\{\s+fnameToDate[^}]+\}\}/)?.length || 0;
        const eq = templateNote.body.match(/\{\{\s+eq[^}]+\}\}/)?.length || 0;
        const getDayOfWeek = templateNote.body.match(/\{\{\s+getDayOfWeek[^}]+\}\}/)?.length || 0;

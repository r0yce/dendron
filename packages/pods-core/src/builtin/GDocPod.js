...
class GDocImportPod extends basev3_1.ImportPod {
    static id = ID;
    static description = "import google doc";
    get config() {
     ...
     */
    getDocumentId = async (accessToken, documentName) => {
        const headers = {
     ...
     */
    fetchDocListFromDrive = async (accessToken) => {
        const headers = {
            Authorization: `Bearer ${accessToken}`,
        };
        const result = await axios_1.default.get(`https://www.googleapis.com/drive/v3/files`, {
     ...
     */
    getAllDocuments = async (accessToken) => {
        let docIdsHashMap;
        let result;
        let error;
        try {
     ...
     */
    getDataFromGDoc = async (opts, config, assetDir) => {
        let response;
     ...
     */
    getCommentsFromDoc = async (opts, response) => {
        let comments;

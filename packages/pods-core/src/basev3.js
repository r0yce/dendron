...
class ExportPod {
    ...
    prepareNotesForExport({ config, notes, }) {
        ...
        if (config.vaults?.exclude) {
            notes = lodash_1.default.reject(notes, (ent) => config.vaults?.exclude?.includes(common_all_1.VaultUtils.getName(ent.vault)));
        }
        if (config.vaults?.include) {
            notes = lodash_1.default.filter(notes, (ent) => config.vaults?.include?.includes(common_all_1.VaultUtils.getName(ent.vault)));
        }
        if (config.ignore) {
